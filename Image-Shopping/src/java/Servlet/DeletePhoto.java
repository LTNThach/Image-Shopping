...
public class DeletePhoto extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int check = DBUtils.DeletePhoto(conn, IDP, loginedUser.getIDU());
        String errorString = null;
        if (check == 0)
        {
            errorString = "Can not delete photo (SQL Problem).";
        }
        session.setAttribute("errorString", errorString);
        
        response.sendRedirect(request.getContextPath() + "/ViewUser?IDU=" + loginedUser.getIDU());
    }
